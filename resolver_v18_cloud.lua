--[[
    FORWARD HVH RESOLVER v18.2 - CLOUD SYNC
    Professional resolver with cloud-based data sharing between teammates
    
    v18.2:
    - Fixed HTTP using require("gamesense/http")
    - Cloud sync now works!
...
function cloud_resolver.report_data(enemy_steam64, angle, confidence, hit, pattern)
    ...
    http.post(url, {["Content-Type"] = "application/json"}, payload, function(data, err)
        if err and err ~= 200 then
            ...
            if CLOUD_CONFIG.DEBUG then
                client.log("[Cloud Resolver] Error: " .. tostring(err))
            end
            return
        end
...
client.log("[Cloud Resolver] HTTP module loaded successfully!")
...
local ui_elements = {
    ...
    reset_btn = ui.new_button("RAGE", "Other", "Reset All Data", function()
        ...
        client.log("[Resolver v18.2] Data reset")
    end)
}
...
client.set_event_callback("paint", function()
    ...
    local x, y = 10, 200
    renderer.text(x, y, 255, 255, 255, 255, "", 0, "══════ RESOLVER v18.2 ══════")
    y = y + 12
...
client.set_event_callback("round_start", function()
    ...
    global_stats.streak_current = 0
    client.log("[Resolver v18.2] Round start - Cloud Ready!")
end)

client.log("[Forward HVH Resolver v18.2] Loaded - Cloud Sync Enabled!")
